import random
import cv2
#random allows for random numbers to be generated in the game, for use in things like damage numbers and chances of encountering enemies.
#cv2 (OpenCV) is imported as it allows for images to be displayed.
wholeGameLoop = True
...
ending12 = False
ending13 = False
# a list of all the endings in the game, and the end counter which is assigned to 0. Each ending variable is assigned as "False", until the players achieve that specific ending, at which point they are assigned as "True". They are placed outside the normal game loop, as it means that not only can the 
#achievements be displayed at the start of every new game, showing what endings they have completed and how many they have gotten in total, but also allow for this to be retained once the players start a new game, meaning the progress is saved
#as long as the players do not exit out of the game.
while wholeGameLoop:
  ...
  def mainMenu(endCount, end1, end2, end3, end4, end5, end6, end7, end8, end9, end10, end11, end12, end13):
    ...
    if endCount == 0:
      ...
      #If the players have not yet gotten an ending to the game, then this message will be displayed.
    else:
      if end1 == True:
      ...
      #displays all endings where the ending variable has been assigned to True, meaning that the players have successfully reached that ending.
  ...
  #calls the mainMenu function, ensuring that its the first thing displayed for the players every time a new game is started.
  ...
  player2Heal = False
  keyItem = False
  #these variables are left empty for each player's statistics. Once each player has confirmed their choice of character to play, then these variables are assigned those character's statistics according to the characterStats nested dictionary.
  characterStats = {
    1: {
      "name":"Gorebrand, Brute of the West",
      "class":"Warrior",
      "weapon":"War Axe",
      "health":20,
      "weaponDmg":"4-7 damage",
      "maxHP":20,
      "weaponMin":4,
      "weaponMax":7,
      "description":"Raised in the bitter sands of Gildor, Gorebrand knows much of hardship. He has sworn not only to raise his lands back from the sea of glass that once swallowed his kin whole, but to defy the great evil that tears this world apart."
    },
    2: {
      "name":"Slakurn, Master of Keys",
      "class":"Rogue",
      "weapon":"Twin daggers",
      "health":25,
      "weaponDmg":"1-9 damage",
      "maxHP":25,
      "weaponMin":1,
      "weaponMax":9,
      "description":"Slakurn was born into a wealthy family in Alenfir, but after learning of their atrocities committed against the people they swore to protect, he ran and became a hero for the weak, and an omen to the wicked."
    },
    3: {
      "name":"Milandril, Protector of the Five",
      "class":"Paladin",
      "weapon":"Greatsword",
      "health":30,
      "weaponDmg":"4-5 damage",
      "maxHP":30,
      "weaponMin":4,
      "weaponMax":5,
      "description":"Once oathbound to the King of Alenfir, Milandril was blamed for the assassination of the very man he swore his life to defend. Now out for answers, he is now on a mission to outrun the approaching Eternal Eclipse to save his kingdom and his friends - no matter the cost."
    },
    4: {
      "name":"Cyrene, Mistress of Time",
      "class":"Sorcerer",
      "weapon":"Mythic Tome",
      "health":20,
      "weaponDmg":"2 - 10 damage",
      "maxHP":20,
      "weaponMin":2,
      "weaponMax":10,
      "description":"A half-human, half-elf woman, Cyrene was gifted with both the power of her elvish brethren, and the ambition of human nature. When her lands were brought down from the sky in the Event of the Falling Horizon, she took it upon herself to seek out the ends of the Earth and become the one true master of time - in the hopes that one day, the Fleeting City would rise again from the ashes."
    }
  }
  ...
  #displays all the names and statistics of each character that the players can choose from.
  print("------------------------------------")
  print("[1] " + warriorName + " [2] " + rogueName + " [3] " + paladinName + " [4] " + sorcererName)
  print("------------------------------------")
  #informs the player what characters there are to select from, and displays the number next to each character so the player knows to input that number into the next part of the game in order to play them.
  ...
  #if the player inputs a number that is listed as one of the four characters to play, then this set of statements will be used to assign that player's variables to the statistics stored in characterStats for the character they selected.
  print("\n \n \n ")
  print("------------------------------------")
  print("PLAYER 1 HAS SELECTED")
  showChar(characterStats, Player1CharChoice)
  #Tells the players what character player 1 selected to play as, as well as show that character's name and statistics from the showChar function.
  print("\n \n \n ")
  print("------------------------------------")
  ...
  while Player2ChoiceLooping:
    try:
      ...
      if Player2CharChoice == Player1CharChoice:
        ...
        #this statement ensures that player 2 cannot pick the same character as player 1, ensuring that playing other characters is encouraged.
      else:
        for things in characterStats:
  ...
  print("\n \n \n ")
  #Tells the players what character player 2 selected to play as, as well as show that character's name and statistics from the showChar function.
  enemyStats = {
    1: {
      "enemyName":"Skeleton ",
      "enemyHealth":10,
  ...
  #enemyStats holds all the statistics for every common enemy in the game. This includes things such as their name, damage amount, and their quips and dialogue. This nested dictionary is used heavily in the combatFunction function.
  bossStats = {
    1:{
  ...
  #bossStats holds all the statistics for every boss enemy in the game, and is only used in the bossFunction function which is only ever called when in a boss room.
  def combatFunction(combat_stats, combat_player1Min, combat_player1Max,combat_player2Min, combat_player2Max, combat_player1Weapon,combat_player2Weapon):
    ...
    global player2Heal
    global keyItem
    #these variables are called so that key information like how much health does each player have, how much is their maximum, as well as the values of their heals and key item slots are known during combat and opening the players' inventories.
    enemyRandom = random.randint(1, 4)
    #enemyRandom is a variable that determines which common enemy out of the 4 in enemyStats will be fighting the players in this combat phase.
  ...
  #this sets variables with appropriate names to the statistics of the enemy that was chosen by enemyRandom, using the enemyStats nested dictionary.
    print("But before you can do anything, a " + enemyName + "appears!")
    print(enemyStartQuip)
    #displays which enemy the players are facing, and prints the enemy's starter piece of dialogue
    if enemyRandom == 1:
      img1 = cv2.imread("Version 4/Images/skeleton.jpg")
      cv2.imshow("Skeleton", img1)
      cv2.waitKey(0)
      cv2.destroyAllWindows()
    elif enemyRandom == 2:
      img2 = cv2.imread("Version 4/Images/rock-golem.jpg")
      cv2.imshow("Rock Golem", img2)
      cv2.waitKey(0)
      cv2.destroyAllWindows()
    elif enemyRandom == 3:
      img3 = cv2.imread("Version 4/Images/slime.jpg")
      cv2.imshow("Slime", img3)
      cv2.waitKey(0)
      cv2.destroyAllWindows()
    elif enemyRandom == 4:
      img4 = cv2.imread("Version 4/Images/zombie.jpg")
      cv2.imshow("Zombie", img4)
      cv2.waitKey(0)
      cv2.destroyAllWindows()
    #Depending upon which enemy is selected to be used for combat (according to the number selection in enemyRandom), that enemy's picture will be displayed using this if/elif set of statements.
    fightHealth = enemyHealth
    talkCountRandom = random.randint(4, 20)
    talkCounter = 0
    #talkCountRandom and talkCounter help to define how many talking attempts both players need to reach in order to convince the enemy to stop attacking. The number has a larger scope of numbers it can select so that combat doesn't feel unneccessary, but talking
    #still seems like a useful tactic if you want to test your luck.
    print("==================================================================")
  ...
  #Informs the enemy which player it is going to attack on that turn, and for how much damage. It then subtracts that set amount of damage from the chosen player's health pool.
      ...
      if quipChoose == 1:
        ...
        print(enemyQuip3)
  #each time an enemy does damage more than 0, it will randomly select one of 3 quips/pieces of dialogue, making each encounter feel slightly more natural and unique each time.
      if player1Health <= 0 and player2Health <= 0:
        ...
        fightLoop = False
        break
  #if both players reach 0 health, then the death message will be displayed and the loop is broken, as well as a random quip selected by the enemy to taunt the fallen players.
      elif enemyRoll == 0:
        print("(The " + enemyName + "didn't really do anything...)")
        print(enemyHitFailQuip)
  #if an enemy does 0 damage, then a unique piece of dialogue will be displayed.
      else:
        pass
      ...
      if player1Health <= 0:
        ...
        while player1Loop:
          ...
          print("\n")
          #displays the player's current Health, and the choices that they can select: talking, opening their inventory or fighting.
          try:
            ...
            if player1Input == "1":
              ...
              if talkCounter == talkCountRandom:
                ...
                player1Loop = False
          #if the player selects "1", then that player will attempt to talk the enemy out of fighting. When this is done, a +1 is added to the talkCounter. the game then sees if the number in talkCounter matches the one in talkCountRandom, and if so will then
          #say that the talk was a success and the combat function will end with the enemy "defeated", as well as the quip for that enemy which displays that it has been a success. Otherwise, the enemy will randomly say a line from one of its talking quips, along
          #with a message telling the player that the enemy is not convinced yet, and that they would need to try again in order to succeed.
            elif player1Input == "2":
              ...
              #if "2" is selected, then the game will show the player's inventory. If they have no key item nor health potion, then the choice to select an item will not be displayed and the game will return the player to the fight selection, 
              #where they can choose talk, fight or attempting to open their inventory again.
              else:
                try:
                  ...
                  if player1InvInput == 1:
                    if player1Heal == True:
                      ...
                      pass
                #if the player selects a health item but do not possess one, then the game will tell them that there is nothing currently there at the moment. However, if they do have a health potion, then the game will ask the player if they wish to use it.
                #if the player says anything other than "y", then the item will not be used, but if the player does input "y", then it will add 5 Health to that player's overall health. The player(playerNumber)MaxHP variable is there to ensure that players cannot
                #heal over their max Health as stated in their character statistics at the start of the game.
                  elif player1InvInput == 2:
                    if keyItem == True:
                      print("Key Items cannot be used in normal combat.")
              ...
              #if "3" is inputted by the player, then the game will determine how much damage they do based off of their character's minimum and maximum damage, and subtract that amount from the enemy's health accordingly.
              #the player will then have one of two different message play, depending on if they did low damage (less than 2) or higher damage (more than 2).
              #the enemy will then say a random piece of dialogue exclaiming their pain, which is decided by the enemyHurt variable.
              ...
              if fightHealth <= 0:
                ...
                print("------------------------")
                print(enemyDeath)
              #if the enemy's health reaches 0, then the message will be display that it has been defeated, along with the enemy's unique death quip.
              ...
              #if one player dies (reaches 0 health) but the other doesnt, once the fight concludes then the alive player will "resurrect" the other, bringing them back into the game again.
                else:
                  pass
                player1Loop = False
              else:
      ...
      if talkCounter == talkCountRandom:
        fightLoop = False
        break
  #if either the enemy's health reaches 0, or the talkCounter becomes equal with the random number chosen in talkCountRandom, then the fight will be concluded and the loop will stop.
      player2Loop = True
    ...
    if player1Health <= 0 and player2Health <= 0:
      ...
      print("==================================================================")
  #if both players die, then the death message will be displayed, and the game will be over - otherwise, it will show that the enemy has been defeated.
    return player1Health
    return player2Health
  #returns the health of both players. If the players both reached 0 health, this will inform the game that they have died and thus will not continue from the room that they died in, instead displaying if they would like to play the game again.
  ...
  def bossFunction(combat_player1Min, combat_player1Max,combat_player2Min, combat_player2Max, combat_player1Weapon,combat_player2Weapon, bossNumber):
    ...
    global keyItem
    #these variables are called so that key information like how much health does each player have, how much is their maximum, as well as the values of their heals and key item slots are known during combat and opening the players' inventories.
    #it also calls the bossStats nested dictionary as well.
  ...
  #assigns appropriately named variables to have the values of the statistics contained within the bossStats nested dictionary. Which boss stats it uses is defined by the bussNumber vairable, which is passed through as a parameter in the boss room the function has been
  #called from.
  
    print(enemyName+ "has appeared!")
    print(enemyStartQuip)
  #displays the name of the enemy, as well as its starting piece of dialogue.
    fightHealth = enemyHealth
    ...
    #if keyItem is equal to True, meaning that the players picked up a key item for this boss, then it will display that it is being used, and that the boss' health is cut in half - as all bosses have 60 health to start off with, when this is done 30 health is removed
    #from the boss before the main fight loop begins.
    fightLoop = True
    while fightLoop:
      print("=================")
      print("[ENEMY'S TURN]")
  ...
  #each time an enemy does damage more than 0, it will randomly select one of 3 quips/pieces of dialogue, making each encounter feel slightly more natural and unique each time.
        ...
        while player1Loop:
          ...
          try:
            ...
            if player1Input == "1":
              ...
              if player1Heal == False and keyItem == False:
                print("You can't use anything right now.")
              #if "1" is selected, then the game will show the player's inventory. If they have no key item nor health potion, then the choice to select an item will not be displayed and the game will return the player to the fight selection, 
              #where they can choose talk, fight or attempting to open their inventory again.
              else:
                try:
                  player1InvInput = int(input("SELECT AN ITEM: [1] HEALTH [2] KEY ITEM: "))
              ...
              if player1Roll < 2:
                ...
                if enemyHurt1 == 1:
                  ...
                  print(enemyHurtDesc2)
              #if "2" is inputted by the player, then the game will determine how much damage they do based off of their character's minimum and maximum damage, and subtract that amount from the enemy's health accordingly.
              #the player will then have one of two different message play, depending on if they did low damage (less than 2) or higher damage (more than 2).
              #the enemy will then say a random piece of dialogue exclaiming their pain, which is decided by the enemyHurt variable.
              fightHealth = fightHealth - player1Roll
              if fightHealth <= 0:
      ...
      if fightHealth <= 0:
        fightLoop = False
        break
      #if the enemy's health reaches 0, then the fight will end, and the loop will stop.
      player2Loop = True
    ...
    if player1Health <= 0 and player2Health <= 0:
      ...
      print("                     [ENEMY DEFEATED!]                            ")
      print("==================================================================")
  #if both player 1 and player 2's health reaches 0, then the fighting loop will break and the death message will be displayed, before the combat and the room function the players were in are exited out of, and are asked if they want to play again.
  #otherwise, the game will display that the enemy has been defeated, and return the players back to the room they were in to receive their ending, which is based upon which room they were in and so, by extension, which boss they defeated.
    return player1Health
    return player2Health
  def looting():
    global player1Heal
    global player2Heal
  #calls the variables for health items for each player.
    ...
    return player1Heal
    return player2Heal
    #a loop that allows for a chance of a healing item to be found. The lootRandom variable rolls a number between 1 and 10, and should the number be equal to 3 or less, then the players find a health potion. They then have the decision to give it to either player 1
    #or player 2, and depending on who gets it their player(playerNumber)Heal variable will be set to true. As such, only one health potion can be present on one player at a time.
    #if the number is 4 or more however, then the function states that the players found nothing of interest, and exits out of the function.
  
  def courtyardStart0():
    img = cv2.imread("Version 4/Images/courtyard.jpg")
    cv2.imshow("Courtyard", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
    ...
    while courtyardLoop:
      ...
      for word in CourtyardInput.split():
        if word in north:
          ...
          westGate4()
          courtyardLoop = False
    #the loop for each room allows for the users to input their desired destination in the game. The users' inputs are split into seperate words, and if one of those words matches with one of the saved words in a given variable, then it will either call the next room
    #for the users to move to and make the loop stop, or if its a looting option allow for the looting function to be called, and still allow the loop to continue, which means the users can input something else to encourage them to move onto the next room in the game.
  ...
  def northBridge1():
    ...
    global player2Heal
    global keyItem
    #These variables are called into the room to allow for the combat function to work properly. They also allow for the looting function to work as well, with the addition of the player1Heal and player2Heal, as well as keyItem variables being called when this function
    #is present in a room.
    img = cv2.imread("Version 4/Images/northBridge.jpg")
    cv2.imshow("Northern Bridge", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
    ...
    #the enemy roll in each roll determines whether or not the players will encounter an enemy or not. When this section is present in a room, there is currently a 50% chance for an enemy to "spawn". If the number is in the range of 1 - 5, then the combat function
    #is called, allowing the players to enter combat. Once the fight ends and the players are victorious, the room will continue to go as normal. If the players die during combat, then the game ends abruptly and they must start from the beginning again.
    roomLoop = True
    while roomLoop:
      print("There are crates situated to the left of you, and a broken down cart to your right. The way is blocked on all sides except for the north, which leads over the bridge and towards the castle atop the hill.")
      print("==========================")
      print("What would you like to do?")
      ...
      for word in northBridgeInput.split():
        if word in north:
          ...
          looting()
          print("You look around further at the broken down cart. The wood has splintered greatly and the wheels seemingly torn off from their axels by a great force. Whatever did this is not something to treat as weak... nor as an ally.")
          print("You move back from the crates and contemplate your options again.")
        elif word in crates:
          looting()
          print("After looking for provisions, you notice a strange mark on the side of the crates - a seared brand depicting a bleeding sun peering down at a vast mountain. You don't know what it means, but it doesn't look very friendly...")
          print("You move away from the cart, being careful not to trip over the decaying bodies of the poor horses that onced pulled it, and start to consider your options again.")
    #the loop for each room allows for the users to input their desired destination in the game. The users' inputs are split into seperate words, and if one of those words matches with one of the saved words in a given variable, then it will either call the next room
    #for the users to move to and make the loop stop, or if its a looting option allow for the looting function to be called, and still allow the loop to continue, which means the users can input something else to encourage them to move onto the next room in the game.
  ...
  def castleGate1():
    ...
    #These variables are called into the room to allow for the combat function to work properly. They also allow for the looting function to work as well, with the addition of the player1Heal and player2Heal, as well as keyItem variables being called when this function
    #is present in a room.
    img = cv2.imread("Version 4/Images/castleGate.jpg")
    cv2.imshow("Castle Gate", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def mainHall1():
    ...
    #These variables are called into the room to allow for the combat function to work properly. They also allow for the looting function to work as well, with the addition of the player1Heal and player2Heal, as well as keyItem variables being called when this function
    #is present in a room.
    img = cv2.imread("Version 4/Images/mainHall.jpg")
    cv2.imshow("Main Hall", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def northChamber1():
    ...
    #These variables are called into the room to allow for the combat function to work properly. They also allow for the looting function to work as well, with the addition of the player1Heal and player2Heal, as well as keyItem variables being called when this function
    #is present in a room.
    img = cv2.imread("Version 4/Images/northChamber.jpg")
    cv2.imshow("North Chamber", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def throneRoom1END():
    ...
    #These variables are called into the room to allow for the combat function to work properly. They also allow for the looting function to work as well, with the addition of the player1Heal and player2Heal, as well as keyItem variables being called when this function
    #is present in a room.
    #The variables ending(number of ending) and endCounter are called and used here, as this is a room that contains one or more endings - as such, these variables are needed to let the game know that once that ending has been achieved, it will display it accordingly on the main menu.
    img = cv2.imread("Version 4/Images/orglond.jpg")
    cv2.imshow("The Throne Room of Orglond the Vile", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
        ...
        if word in fight:
          ...
          if player1Health <= 0 and player2Health <= 0:
            ...
            if ending1 == False:
              ...
              ending1 = True
            #Prints the ending screen and dialogue for the players to see, and then checks to see if the ending variable assigned with this ending is True or False. If the variable is assigned as False, then the endCounter will go up by 1
            #and the ending variable assigned to True - this then tells the game that the players have now achieved this ending, and should they get it again it will not count towards the total amount of endings completed on the main menu.
            roomLoop = False 
        elif word in north:
          northOfNorth1END()
          roomLoop = False
    #the loop for each room allows for the users to input their desired destination in the game. The users' inputs are split into seperate words, and if one of those words matches with one of the saved words in a given variable, then it will either call the next room
    #for the users to move to and make the loop stop, or if its a looting option allow for the looting function to be called, and still allow the loop to continue, which means the users can input something else to encourage them to move onto the next room in the game.
    #In this case, the choices are either to fight the boss, or go to the other secret ending.
  def northOfNorth1END():
    global ending2
    global endCounter
    #The variables ending(number of ending) and endCounter are called and used here, as this is a room that contains one or more endings - as such, these variables are needed to let the game know that once that ending has been achieved, it will display it accordingly on the main menu.
    img = cv2.imread("Version 4/Images/northOfNorth.jpg")
    cv2.imshow("North of the North", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def eastChamber2():
    ...
    #These variables are called into the room to allow for the combat function to work properly. They also allow for the looting function to work as well, with the addition of the player1Heal and player2Heal, as well as keyItem variables being called when this function
    #is present in a room.
    img = cv2.imread("Version 4/Images/eastChamber.jpg")
    cv2.imshow("East Chamber", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def dungeons2END():
    ...
    global endCounter
    #The variables ending(number of ending) and endCounter are called and used here, as this is a room that contains one or more endings - as such, these variables are needed to let the game know that once that ending has been achieved, it will display it accordingly on the main menu.
    img = cv2.imread("Version 4/Images/dungeon.jpg")
    cv2.imshow("Dungeon", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def deeperDungeon2END():
    ...
    global endCounter
    #The variables ending(number of ending) and endCounter are called and used here, as this is a room that contains one or more endings - as such, these variables are needed to let the game know that once that ending has been achieved, it will display it accordingly on the main menu.
    img = cv2.imread("Version 4/Images/deeperDungeon.jpg")
    cv2.imshow("Deeper Dungeon", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def mountainCrack3():
    ...
    #These variables are called into the room to allow for the combat function to work properly. They also allow for the looting function to work as well, with the addition of the player1Heal and player2Heal, as well as keyItem variables being called when this function
    #is present in a room.
    img = cv2.imread("Version 4/Images/mountainCrack.jpg")
    cv2.imshow("Mountain Crack", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def crystalCave3():
    ...
    #These variables are called into the room to allow for the combat function to work properly. They also allow for the looting function to work as well, with the addition of the player1Heal and player2Heal, as well as keyItem variables being called when this function
    #is present in a room.
    img = cv2.imread("Version 4/Images/crystalCave.jpg")
    cv2.imshow("Crystal Cave", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def fishingWorld3END():
    ...
    #is present in a room.
    #The variables ending(number of ending) and endCounter are called and used here, as this is a room that contains one or more endings - as such, these variables are needed to let the game know that once that ending has been achieved, it will display it accordingly on the main menu.
    img = cv2.imread("Version 4/Images/bigCatch.jpg")
    cv2.imshow("The Big Catch", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def pitOfEternity3():
    ...
    #These variables are called into the room to allow for the combat function to work properly. They also allow for the looting function to work as well, with the addition of the player1Heal and player2Heal, as well as keyItem variables being called when this function
    #is present in a room.
    img = cv2.imread("Version 4/Images/pitOfEternity.jpg")
    cv2.imshow("The Pit of Eternity", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def hollowEarth3END():
    ...
    global ending6
    #The variables ending(number of ending) and endCounter are called and used here, as this is a room that contains one or more endings - as such, these variables are needed to let the game know that once that ending has been achieved, it will display it accordingly on the main menu.
    img = cv2.imread("Version 4/Images/hollowEarth.jpg")
    cv2.imshow("The Hollow Earth", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def westGate4():
    ...
    #These variables are called into the room to allow for the combat function to work properly. They also allow for the looting function to work as well, with the addition of the player1Heal and player2Heal, as well as keyItem variables being called when this function
    #is present in a room.
    img = cv2.imread("Version 4/Images/westGate.jpg")
    cv2.imshow("West Gate", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def abyssRealm4END():
    ...
    global ending7
    #The variables ending(number of ending) and endCounter are called and used here, as this is a room that contains one or more endings - as such, these variables are needed to let the game know that once that ending has been achieved, it will display it accordingly on the main menu.
    img = cv2.imread("Version 4/Images/abyssRealm.jpg")
    cv2.imshow("The Shadow Realm", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def westTown4():
    ...
    #These variables are called into the room to allow for the combat function to work properly. They also allow for the looting function to work as well, with the addition of the player1Heal and player2Heal, as well as keyItem variables being called when this function
    #is present in a room.
    img = cv2.imread("Version 4/Images/westTown.jpg")
    cv2.imshow("Town Square", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def oldTemple4():
    ...
    #These variables are called into the room to allow for the combat function to work properly. They also allow for the looting function to work as well, with the addition of the player1Heal and player2Heal, as well as keyItem variables being called when this function
    #is present in a room.
    img = cv2.imread("Version 4/Images/oldTemple.jpg")
    cv2.imshow("The Old Temple", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def lordOfStone4END():
    ...
    #is present in a room.
    #The variables ending(number of ending) and endCounter are called and used here, as this is a room that contains one or more endings - as such, these variables are needed to let the game know that once that ending has been achieved, it will display it accordingly on the main menu.
    img = cv2.imread("Version 4/Images/lordOfStone.jpg")
    cv2.imshow("The Lord of Stone", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def sacredAltar4():
    ...
    #These variables are called into the room to allow for the combat function to work properly. They also allow for the looting function to work as well, with the addition of the player1Heal and player2Heal, as well as keyItem variables being called when this function
    #is present in a room.
    img = cv2.imread("Version 4/Images/sacredAltar.jpg")
    cv2.imshow("The Sacred Altar", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def sacrifice4END():
    ...
    global ending9
    #The variables ending(number of ending) and endCounter are called and used here, as this is a room that contains one or more endings - as such, these variables are needed to let the game know that once that ending has been achieved, it will display it accordingly on the main menu.
    img = cv2.imread("Version 4/Images/sacrifice.jpg")
    cv2.imshow("Sacrifice", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def endlessDunes4():
    ...
    #These variables are called into the room to allow for the combat function to work properly. They also allow for the looting function to work as well, with the addition of the player1Heal and player2Heal, as well as keyItem variables being called when this function
    #is present in a room.
    img = cv2.imread("Version 4/Images/endlessDunes.jpg")
    cv2.imshow("The Endless Dunes", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def procedural4END():
    ...
    global ending10
    #The variables ending(number of ending) and endCounter are called and used here, as this is a room that contains one or more endings - as such, these variables are needed to let the game know that once that ending has been achieved, it will display it accordingly on the main menu.
    img = cv2.imread("Version 4/Images/procedural.jpg")
    cv2.imshow("Procedural Generation", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def westShack5():
    ...
    #These variables are called into the room to allow for the combat function to work properly. They also allow for the looting function to work as well, with the addition of the player1Heal and player2Heal, as well as keyItem variables being called when this function
    #is present in a room.
    img = cv2.imread("Version 4/Images/westShack.jpg")
    cv2.imshow("The Shack", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def despairShack5END():
    ...
    global ending11
    #The variables ending(number of ending) and endCounter are called and used here, as this is a room that contains one or more endings - as such, these variables are needed to let the game know that once that ending has been achieved, it will display it accordingly on the main menu.
    img = cv2.imread("Version 4/Images/despairShack.jpg")
    cv2.imshow("Going up from here", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def shackField5():
    ...
    #These variables are called into the room to allow for the combat function to work properly. They also allow for the looting function to work as well, with the addition of the player1Heal and player2Heal, as well as keyItem variables being called when this function
    #is present in a room.
    img = cv2.imread("Version 4/Images/shackField.jpg")
    cv2.imshow("Field of Shacks", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def splinterPlains5():
    ...
    #These variables are called into the room to allow for the combat function to work properly. They also allow for the looting function to work as well, with the addition of the player1Heal and player2Heal, as well as keyItem variables being called when this function
    #is present in a room.
    img = cv2.imread("Version 4/Images/splinterPlains.jpg")
    cv2.imshow("The Splintered Plains", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  def shacktronic5END():
    ...
    #is present in a room.
    #The variables ending(number of ending) and endCounter are called and used here, as this is a room that contains one or more endings - as such, these variables are needed to let the game know that once that ending has been achieved, it will display it accordingly on the main menu.
    img = cv2.imread("Version 4/Images/shacktronic.jpg")
    cv2.imshow("The Shack O'Tronic", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #The variable img is saved as the file destination of the image needed to be displayed in this room. cv2.imshow() is then what allows the image to be displayed, using both the img variable and a custom string as the window name when it displays.
    #The waitKey function is then set to 0, meaning that the window will not close after any specific amount of time, and the destroyAllWindows function then acts to allow the user to finally exit the image window, as soon as any keyboard input is detected.
  ...
  courtyardStart0()
  #the courtyard function is called here so that it will always be the first room the players enter into.
  ...
  if playAgainInput == "y":
    ...
    if playWarning == "y":   
      ...
      wholeGameLoop = True
  #A loop is set up here so that the players can choose whether or not they want to play the game again. If they input "y", then the game will restart back to the mainMenu function, otherwise if they input anything else then they will be asked
  #if they really want to stop playing, as then achievement progress will not be retained if they start the game again. If they input "y", the game restarts, and if they input anything else then it will tell them "Thank you for playing!" and
  #the gameloop stops.